...
const App = () => {
  ...
  useEffect(() => {
    if (selectedMode === 'historical') {
      setHistoricalData(actualHistoricalData);
    }
  }, [selectedMode]);
  ...
  const readNfc = async () => {
    try {
      ...
      const vals = [];
      for (let i = 0; i < data.length; i += 2) vals.push((data[i + 1] << 8) | data[i]);
      const [b0, b1, b2, b3] = vals;
      const wordVals = vals.slice(4, 110);
      const cb0 = b0 | (b1 << 16), cb1 = b2 | (b3 << 16);
      const disp0 = statusBitLabels0.map((l, i) => ({ label: l, state: (cb0 & (1 << i)) ? 'ON' : 'OFF' }));
      const disp1 = statusBitLabels1.map((l, i) => ({ label: l, state: (cb1 & (1 << i)) ? 'ON' : 'OFF' }));
      const wp = wordVals.map((v, i) => wordLabels[i] ? { label: wordLabels[i], value: v } : null).filter(x => x);
